...
`;
  ...
  test('It should get item', () => {
    ...
    expect(tree.get(4).getHtml()).toBe('<p></p>');
  });

  test('It should nested item parse', () => {
    expect(tree.get(12).getHtml()).toBe(
  ...
  test('It should show tree', () => {
    const html =
      '<h2>애국가</h2><p><a href="https://www.naver.com">naver</a></p><p><img src="hello.png" alt="example" /></p><hr/><p></p><ol><li>동해물과 백두산이</li><li>마르고 닳도록</li></ol><ul><li>하느님이 보우하사</li><li>우리 나라 만세</li></ul><blockquote>무궁화 삼천리 화려강산</blockquote><p>대한사람 대한으로 길이 보전하세</p><p></p><ol><li>남산 위의 <em>저 소나무</em> 철갑을 두른 듯</li></ol><p></p>';
